...
class CorrectLead extends Command
{
    ...
    {
        $oldLeads = DB::connection('richxyz')->table('network_clicks')->join('clicks', 'network_clicks.sub_id', '=', 'clicks.hash_tag')
        ...
        foreach ($oldLeads as $oldLead) {
            DB::connection('richxyz')->table('network_clicks')->where('id', $oldLead->lead_id)->update([
